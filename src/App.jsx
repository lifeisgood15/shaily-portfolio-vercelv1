  ...
  return (
    <div className="min-h-screen pb-12 px-4 md:px-8 lg:px-16 max-w-5xl mx-auto pt-24 relative">
      ...
      <main className="space-y-16">
        ...
        <div id="journey" className="space-y-12 scrapbook-border p-6 md:p-8 tape-pink shadow-scrapbook bg-white scroll-mt-24">
          <h2 className="text-2xl font-bold text-ink-dark border-b-2 border-paper-dark pb-2 inline-block">My Journey</h2>
      ...
      <footer id="contact" className="mt-20 pb-12 scroll-mt-24 w-full">
        ...
        <div className="border-t-2 border-paper-dark pt-8 w-full">
          <p className="text-xs font-semibold uppercase tracking-widest text-ink-light mb-4">Get in touch</p>
          <div className="flex flex-row flex-wrap items-center gap-x-6 gap-y-3">
            <h1 className="text-sm font-medium text-ink-dark">Lets talk over your <span style={{ color: "#8a4e54", fontWeight: "bold" }}>[insert favourite beverage]</span></h1>
            ...
            {portfolioData.contact.methods.map((method, index) => {
              ...
              const linkHref = isEmail ? `mailto:${method.handle}` : method.handle;
              return (
                <a
                  key={index}
                  href={linkHref}
          ...
          <div className="mt-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 text-xs text-ink-light">
            <p>© {portfolioData.name} {new Date().getFullYear()}</p>
