...
const Navigation = ({ portfoliodata }) => {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 bg-paper-texture shadow-sm border-b border-paper-dark/20" style={{ height: '3.5rem' }}  >
      <div className="max-w-5xl mx-auto px-4 md:px-8 lg:px-16 flex justify-between items-start md:items-center h-16">
        ...
        <div className="flex items-center h-full">
          <span className="font-bold text-ink-dark text-lg md:hidden">Shaily Pandey</span>
        </div>
        ...
        <nav className="hidden md:flex h-full items-start">
          {links.map((link) => (
            <button
              ...
              style={{ fontFamily: "'Caveat', 'Kalam', 'Patrick Hand', cursive, sans-serif" }}
            >
              {link.name}
            </button>
          ))}
