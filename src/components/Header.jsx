...
import { Mail, Linkedin, Github, Youtube, CalendarDays, ExternalLink, Heart } from 'lucide-react';
...
const getIcon = (platform) => {
  ...
  if (p.includes('meet') || p.includes('calendar')) return <CalendarDays className="w-4 h-4" />;
...
const getStyle = (platform) => {
  const p = platform.toLowerCase();
  if (p === 'email') return 'bg-white border border-paper-dark text-ink hover:shadow-md';
  if (p === 'linkedin') return 'bg-[#0077b5]/10 border border-[#0077b5]/30 text-[#0077b5] hover:bg-[#0077b5]/20';
  if (p === 'github') return 'bg-[#1a1a2e]/10 border border-[#1a1a2e]/20 text-[#1a1a2e] hover:bg-[#1a1a2e]/20';
  if (p === 'youtube') return 'bg-red-50 border border-red-200 text-red-600 hover:bg-red-100';
        ...
        <div className="flex-1 space-y-4 text-center md:text-left mt-4 md:mt-0">
          <div className="flex items-center justify-center md:justify-start gap-2">
            <h1 className="text-4xl md:text-5xl font-bold text-ink-dark tracking-tight">
              Hi, I'm <span className="text-[#d87d85]">{data.name.split(' ')[0]}!</span>
            </h1>
