...
import { ArrowRight, Lightbulb, TrendingUp, ChevronDown, ChevronUp, PlayCircle, ImageOff } from 'lucide-react';

// Import all images from project-backdrops at build time (Vite requirement for src/ assets)
const projectImages = import.meta.glob('../resources/project-backdrops/*', { eager: true });
...
const Projects = ({ projects }) => {
  ...
  const cardColors = ['bg-[#fde7eb]', 'bg-[#e5effa]', 'bg-[#e7f5ed]', 'bg-[#fff5e6]'];
  ...
  return (
    <div className="py-8">
      <div className="flex items-center gap-4 mb-8">
        <h2 className="text-2xl font-bold text-ink-dark border-b-2 border-paper-dark pb-2">Case Studies</h2>
          ...
          return (
            ...
            >
              ...
              <div className="p-6 flex-1 flex flex-col">
                <h3 className="text-xl font-bold text-ink-dark mb-1 leading-tight">{project.title}</h3>
                <p className="text-sm font-medium text-ink-light mb-6">{project.role}</p>

                <div className="space-y-4 flex-1">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <Lightbulb className="w-4 h-4 text-ink-dark" />
                      <h4 className="text-sm font-bold text-ink-dark">Summary</h4>
                    </div>
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <h4 className="text-sm font-bold text-ink-dark">Impact</h4>
                    </div>
                    <p className="text-sm text-ink leading-relaxed border-l-2 border-paper-dark/30 pl-3 ml-2">
                      {project.impact}
                    </p>
                ...
                {hasButtons && (
                  <div className="mt-8 pt-4 border-t border-paper-dark/10 flex flex-col sm:flex-row gap-3">
                    {showDemo && (
                      <button className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-ink-dark text-white rounded-md text-sm font-bold shadow hover:bg-ink transition-colors group/btn" onClick={() => window.open(project['demo-url'], "_blank")}>
